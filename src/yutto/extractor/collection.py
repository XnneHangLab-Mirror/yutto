...
class CollectionExtractor(BatchExtractor):
    ...
    async def extract(
        ...
        avids = [item["avid"] for item in collection_details["pages"]]
        ugc_video_lists = await resolve_ugc_video_lists(ctx, client, avids)

        for avid, ugc_video_list in zip(avids, ugc_video_lists, strict=True):
            if ugc_video_list is None:
                continue
            if len(ugc_video_list["pages"]) != 1:
                Logger.error(f"视频合集 {collection_title} 中的视频 {avid} 包含多个视频！")
